...
/* -----------------------------------------------------
  ...
  if (g_dir == IDIR){
    ...
    /* -- I1. initialize rhs with flux difference -- */
      ...
      //NVAR_LOOP(nv) rhs[i][nv] = - dtdx*(state->Dm[i][nv] + state->Dp[i-1][nv]);// - dtdx*(flux[i][nv] - flux[i-1][nv]);
      #if USE_PR_GRADIENT == YES
      rhs[i][MX1] -= dtdx*(p[i] - p[i-1]);
      #endif
      
      #if CR_FLUID == NC_PdV_TOTENG // NEW            
         rhs[i][ECR] -= CR_Source(i, state, vp, vm, dtdx, 1., A);
      #endif      
    ...
    /* -- J1. initialize rhs with flux difference -- */
      ...
      #if CR_FLUID == NC_PdV_TOTENG // NEW       
         rhs[j][ECR] -= CR_Source(j, state, vp, vm, dtdx, 1., A);
      #endif 
    ...
    /* -- K1. initialize rhs with flux difference -- */
      ...
      #if CR_FLUID == NC_PdV_TOTENG  // NEW
         rhs[k][ECR] -= CR_Source(k, state, vp, vm, dtdx, 1., A);
      #endif 
...
*/
    ...
    /* -- K1.  initialize rhs with flux difference -- */
      ...
      #if CR_FLUID != NO // NEW
       if (CR_FLUID == NC_PdV_TOTENG){
         rhs[k][ECR] -= CR_Source(k, state, vp, vm, dtdx, dtdV, A);
       }
